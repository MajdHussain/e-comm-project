...
@for ($i=1*$id; $i<1*$id+15; $i++)
@if (isset($Product[$i-15]))
<div class="tredning-item">
  <a href="/detailAdmin/{{$Product[$i-15]['id']}}">
  <img class="tredning-img" src="{{$Product[$i-15]['gallery']}}">
  <div >
    <h5>{{$Product[$i-15]['name']}}</h5>
    <p>{{$Product[$i-15]['price']}}$</p>
  </div>
  </a>
</div>
@endif
...
<a class="btn btn-primary" href="/Apages/{{{$size * 15}}}">{{$size}}</a>
@endfor
</div>
@endsection 